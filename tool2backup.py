...
from lib.dres import send_image
...
def search_clip_shortcut():
  ...
  top_result = logic.model.search_clip(text)
...
def hide_images(start_idx):
  for i in range(start_idx, shown):
...
def score_search_shortcut():
  ...
  logic.model.update_scores(logic.history[-1], likeID, 0.5)
...
with dpg.texture_registry():
  for row in range(button_rows):
    for col in range(button_cols):
      dpg.add_dynamic_texture(width=button_width, height=button_height, default_value=logic.get_dpg_image(row * button_cols + col), tag=get_texture_tag(row, col))
